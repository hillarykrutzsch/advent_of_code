...
for(let x = 0; x < input1Array.length; x++){
	let direction = input1Array[x].substring(0,1);
	let steps = parseInt(input1Array[x].substring(1));
	let lastCursor = 0;
	switch(direction){
		case 'L':
			for(var q = cursorX-1; q >= cursorX-steps; q--){
				//array2D[q][cursorY] = 1;
				lengthCount++;
				hashMap[`${q},${cursorY}`] = lengthCount;
				lastCursor = q;
			}
			cursorX = lastCursor;
		break;
		...
		break;
		case 'U':
			for(var q = cursorY+1; q <= cursorY+steps; q++){
				//array2D[cursorX][q] = 1;
				lengthCount++;
				hashMap[`${cursorX},${q}`] = lengthCount;
				lastCursor = q;
			}
			cursorY = lastCursor;
		break;
...
for(let x = 0; x < input2Array.length; x++){
	let direction = input2Array[x].substring(0,1);
	let steps = parseInt(input2Array[x].substring(1));
	let lastCursor = 0;
	switch(direction){
		case 'L':
			for(var q = cursorX-1; q >= cursorX-steps; q--){
				lengthCount2++;
				//if(array2D[q][cursorY] == 1){
				if(hashMap[`${q},${cursorY}`]){	
					console.log('INTERSECTION!: array2D['+q+']['+cursorY+']');

					intersectionArray.push([q,cursorY, lengthCount2+hashMap[`${q},${cursorY}`]]);
				}
				lastCursor = q;
			}
			cursorX = lastCursor;
		...
		case 'U':
			for(var q = cursorY+1; q <= cursorY+steps; q++){
				lengthCount2++;
				//if(array2D[cursorX][q] == 1){
				if(hashMap[`${cursorX},${q}`]){	
					console.log('INTERSECTION!: array2D['+cursorX+']['+q+']');
					intersectionArray.push([cursorX,q, lengthCount2+hashMap[`${cursorX},${q}`]]);
				}
				lastCursor = q;
			}
			cursorY = lastCursor;
...
let bestManhattanDistance = ARRAYSIZE;
let shortestLength = intersectionArray[0][2];

for(let index = 0; index < intersectionArray.length; index++){
	let coorX = intersectionArray[index][0];
	let coorY = intersectionArray[index][1];
	let xDistanceFromOrigin = Math.abs(coorX - (ORIGINX));
	let yDistanceFromOrigin = Math.abs(coorY - (ORIGINY));
	console.log('coorX: ' + coorX);
	console.log('coorY: ' + coorY);
	console.log('xDistanceFromOrigin: ' + xDistanceFromOrigin);
	console.log('yDistanceFromOrigin: ' + yDistanceFromOrigin);
	let manhattanDistance = xDistanceFromOrigin+yDistanceFromOrigin;
	if(manhattanDistance < bestManhattanDistance){
		bestManhattanDistance = manhattanDistance;
	}
	if(intersectionArray[index][2] < shortestLength){
		shortestLength = intersectionArray[index][2];
	}
}

console.log('bestManhattanDistance: ' + bestManhattanDistance);
